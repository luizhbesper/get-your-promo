    ...
    return (
        <div className="flex flex-col items-center justify-center h-screen">
            <h1 className="text-2xl md:text-3xl font-bold text-left md:w-xl lg:w-3xl">
                Hi {track.receiver}, hope you like it :)
            </h1>
            <div className=" bg-zinc-900 md:w-xl lg:w-3xl rounded-lg mt-4 p-6 flex gap-x-8">
                <div className="w-48 h-48 relative flex items-center justify-center">
                    ...
                    >
                        {isPlaying ? (
                            <Pause className="" size={32} weight="fill" />
                        ) : (
                            <Play className="" size={32} weight="fill" />
                        )}
                    </button>
                </div>
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            {track.tags.map((tag, index) => (
                                <span
                                    key={index}
                                    className="mr-2 text-sm bg-cyan-800 text-zinc-200 px-2 rounded-full"
                                >
                                    {tag}
                                </span>
                            ))}
